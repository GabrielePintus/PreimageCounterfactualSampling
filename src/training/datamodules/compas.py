...
_CSV_NAME = "compas-scores-two-years.csv"
...
class CompasDataModule(L.LightningDataModule):
    ...
    def prepare_data(self) -> None:
        """Download compas-scores-two-years.csv if not already present."""
        import urllib.request

        dest = Path(self.hparams.data_dir) / "Compass" / _CSV_NAME
        if not dest.exists():
            dest.parent.mkdir(parents=True, exist_ok=True)
            print(f"[CompasDataModule] Downloading {_CSV_NAME} ...")
            urllib.request.urlretrieve(_PROPUBLICA_URL, dest)
            print(f"[CompasDataModule] Saved to {dest}")

    def setup(self, stage=None) -> None:
        import pandas as pd

        csv_path = Path(self.hparams.data_dir) / "Compass" / _CSV_NAME
        df = pd.read_csv(csv_path)

        # Standard ProPublica row filters
        df = df[
