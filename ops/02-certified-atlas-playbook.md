...
In the benchmark path, training support labels are computed once from the
benchmark model's **predicted** classes and then passed into
`counterfactuals.methods.certcf.CertCF.fit(...)`. The wrapper treats those
incoming labels as authoritative support labels when building the atlas.
