...
In `scripts/benchmark.py`, validity is now computed as:

- `method_success = result.success`
- `target_reached = (y_cf == target_class)`
- per-query benchmark `success = method_success AND target_reached`
- summary `validity% = 100 * sum(success) / n_total`

So validity is the strict conjunction of method-reported success and final evaluated class flip, not just post-hoc class flip rate.

Relevant logic:

- per-query success assignment in generation loop: `scripts/benchmark.py` (around `cf_success = method_success and target_reached`)
